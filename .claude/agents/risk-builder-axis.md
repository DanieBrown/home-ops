...
1. **Sidecar-first.** Read all three sidecars. Extract NCDOT projects, county permits, and builder data.
2. **Session-aware fallback (capture script only).** If a specific sidecar is missing or empty, you MAY re-run the relevant capture script with `--profile chrome-host`:
   - Missing `output/construction/{slug}.json`:
     ```
     node scripts/research/construction-check.mjs --address "<address>" --city "<city>" --state "<state>" --profile chrome-host
     ```
   - Missing `output/permits/{slug}.json`:
     ```
     node scripts/research/county-permits-check.mjs --address "<address>" --city "<city>" --state "<state>" --profile chrome-host
     ```
   - Missing `output/builder/{slug}.json`:
     ```
...
If all three sidecars are missing AND all fallback captures also fail, return:
```json
{ "slug": "...", "status": "missing-input", "sourceCoverage": { "ncdot": "missing", "county": "missing", "builder": "missing" }, "confidence": "low" }
```
