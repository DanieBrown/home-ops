...
Portal pages (Facebook, Nextdoor, etc.) require authentication. A fresh browser has no cookies and will receive 403s or empty pages. The hosted Chrome session at `--profile chrome-host` is already logged in. The capture scripts below connect to it via CDP — do not attempt to open any browser yourself.
...
2. **Session-aware fallback (capture script only).** If `output/sentiment/{slug}.json` is missing, empty, or its `status` is `"missing-input"` / `"failed"`, you MAY re-run the public sentiment capture against the established session:
   ```
...
- **Do NOT** launch a browser yourself or use Playwright directly — a new session has no auth and will 403.
