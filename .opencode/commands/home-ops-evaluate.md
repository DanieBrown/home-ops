...
Prefer the checked-in `node evaluate-pending.mjs` or `npm.cmd run evaluate:pending` path for the deterministic packet/extraction/orchestration work so evaluate reuses the repo implementation instead of recreating temporary helper scripts.
