...
6. Capture assigned-school metadata from the listing source or district assignment lookup first, then GreatSchools/configured school metadata sources for rating, enrollment, student/teacher ratio, and ethnicity distribution when available.
...
12. Run `node scripts/research/research-coverage-audit.mjs <report-path>` or `npm.cmd run audit:research -- <report-path>` and surface any explicit neighborhood, school, or development evidence gaps in the summary.
...
9. If the `Evaluate Worker` agent is unavailable or unreliable in the current environment, preserve the same one-home packet contract and use `scripts/pipeline/evaluate-pending.mjs` to materialize the reports directly instead of improvising new multi-home helper scripts.
...
12. After each worker returns, stage one tracker addition per handled property under `batch/tracker-additions/` using the canonical 11-column home-ops TSV order. Then merge with `node scripts/pipeline/merge-tracker.mjs --verify` or `npm.cmd run merge -- --verify` on Windows PowerShell.
...
16. Open that saved top-10 cohort in the hosted Chrome session inside one tab group named `Top 10`. Prefer each report's `**URL:**` field and fall back to the report file only when a direct listing URL is missing. Use `node scripts/browser/review-tabs.mjs shortlist-top10 --group "Top 10"` or `npm.cmd run browser:review -- shortlist-top10 --group "Top 10"` on Windows PowerShell. If the hosted Chrome session is closed, reopen it before opening the review group.
...
19. After the reports are written, run `scripts/research/research-coverage-audit.mjs` against the just-generated report set and report whether neighborhood, school, and development coverage was explicit, weak, or missing.
20. When the audit flags school or development gaps, run `scripts/research/research-source-plan.mjs` on the affected reports so the next pass uses the exact `portals.yml` sources and suggested lookup targets rather than ad hoc searching.
...
For batch evaluation, prefer staged TSV additions plus `scripts/pipeline/merge-tracker.mjs` over concurrent direct edits to `data/listings.md`.
...
- Treat the configured `sentiment_sources`, `school_sources`, and `development_sources` inventories in `portals.yml` as required research surfaces, not decorative config. Use `scripts/research/research-source-plan.mjs` to resolve the school and development inventories into actual source targets, and if a source class was not actually used, say so plainly and let the research audit reflect that gap.
