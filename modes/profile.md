...
Follow this flow exactly. The key rule: the wizard server MUST be launched as a **foreground** Bash call, not a background one. Background launches let you end your turn before the user submits, which breaks auto-pickup; a foreground call blocks your turn until the `--once` server exits on submit, which is the mechanical guarantee that you wait for the user.

1. **Announce the URL first.** Before blocking on the server, tell the user in one short line: "Wizard will open at http://127.0.0.1:4178/ -- fill it out and click Submit; I'll pick it up automatically." If Chrome MCP is available, also open the URL in the hosted Chrome session now so the tab is ready before the server starts accepting submissions.
2. **Launch the server in the foreground with a long Bash timeout.** Call the Bash tool with:
   - `command`: `node tools/profile-wizard/serve.mjs --once --port 4178`
   - `timeout`: `600000` (10 minutes, Bash's maximum)
   - **Do NOT set `run_in_background: true`.** Foreground is mandatory -- that is the mechanism that holds your turn open while the user works through the wizard.
   The server prints its banner, then the Node process stays alive until the user submits. On submit it writes `.home-ops/profile-wizard-submission.json` and exits ~250ms later, which returns control to you with the submission file already on disk.
3. **Handle the 10-minute cap with a retry loop.** If Bash times out before the submission file exists, the user is still filling in the wizard. Do not prompt them. Re-launch the server with the same command (the wizard page reseeds from `.home-ops/profile-wizard-answers.json` on refresh, so the user just reloads the tab and keeps going). Cap the retries at three (≈30 minutes total). If after three rounds the submission file still does not exist, only then ask the user whether they want to keep going or switch to the in-chat flow.
4. **Ingest the moment the submission file exists.** As soon as the foreground Bash call returns and `.home-ops/profile-wizard-submission.json` is present, read it and map the answers into `config/profile.yml`, `buyer-profile.md`, and `modes/_profile.md` using the file-update rules below. Do not ask the user to say "finish my profile from the wizard submission" -- that phrase is only a manual fallback for when auto-pickup fails. Then delete or rename the submission file so the next run starts clean.
