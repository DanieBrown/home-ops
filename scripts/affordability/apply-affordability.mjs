...
function affordabilityNote(result) {
  ...
  return [
    ...
    '',
    `- Conservative profile range ceiling: ${formatMoney(result.recommended_price_max)}.`,
    ...
    `- Binding constraint: ${selected.constraint === 'cash_available' ? 'cash available for down payment and closing costs' : '25% monthly housing-payment cap'}.`,
...
function profileModeNote(result) {
  ...
  return [
    ...
    '',
    `Use ${formatMoney(result.recommended_price_max)} as the current conservative price ceiling unless the buyer explicitly revises affordability.`,
